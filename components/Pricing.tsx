...
const PricingCard = ({ tier, price, features, recommended, annual, ctaLink, ctaText, disabled, trial }: any) => {
    const displayPrice = annual ? Math.round(price * 0.8) : price;
    
    return (
        <motion.div 
            whileHover={disabled ? {} : { y: -8 }}
            className={`relative flex flex-col p-8 rounded-[2rem] h-full overflow-hidden transition-all duration-300 ${recommended ? 'bg-zinc-900/80 border border-blue-500/50' : 'glass-card'} ${disabled ? 'opacity-70' : ''}`}
        >
            ...
            <div className="mb-8">
                ...
                {trial && (
                    <div className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-blue-400 px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/20">
            ...
            {disabled ? (
                 <button disabled className="w-full py-4 rounded-xl font-bold bg-white/5 text-slate-500 border border-white/5 cursor-not-allowed">
                    {ctaText}
                 </button>
            ) : (
                <a 
                    ...
                    className={`w-full py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-sm tracking-wide ${
                    recommended 
...
const Pricing: React.FC = () => {
  // Changed default to false (Monthly) as requested
  const [annual, setAnnual] = useState(false);

  // Dynamic links based on billing cycle
  const startLink = annual 
    ? "https://buy.stripe.com/3cIaEXgxOfg0aWvaP5bfO02"
    : "https://buy.stripe.com/fZu7sL81i2tefcL0arbfO00";

  const proLink = annual
    ? "https://buy.stripe.com/6oU3cvgxOebW7Kj9L1bfO03"
    : "https://buy.stripe.com/7sY14nchy6Ju7Kj1evbfO01";

  return (
    <section id="pricing" className="w-full px-6 max-w-7xl mx-auto py-20 relative z-10">
      
      <div className="text-center mb-12 relative z-10">
        <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Tarification Simple</h2>
        <p className="text-slate-400 text-lg mb-8 max-w-2xl mx-auto">Commencez par <span className="text-white font-bold">7 jours gratuits</span>. Annulez à tout moment.</p>
